...
class Contenedor {
    constructor() {
        this.filePath = '../db/productos.json';
    }

    getAll = async () => {
        try {
            const productos = await fs.promises.readFile(this.filePath);
    ...
    getById = async (id) => {
        try {
            ...
            return console.log(`Producto encontrado con el id ${id}: ${JSON.stringify(productoEncontrado)}`);
